...
HAL_StatusTypeDef get_accel(double accel[3]) {
	...
	accel[0] = (accel_buf[0] << 8) | (accel_buf[1]) / MPU_ACCEL_RANGE_8G * GS_TO_MPSPS;
	accel[1] = (accel_buf[2] << 8) | (accel_buf[3]) / MPU_ACCEL_RANGE_8G * GS_TO_MPSPS;
	accel[2] = (accel_buf[4] << 8) | (accel_buf[5]) / MPU_ACCEL_RANGE_8G * GS_TO_MPSPS;
	return HAL_OK;
}
